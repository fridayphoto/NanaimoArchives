...
<div id="search_result" style="padding:30px 0px 50px 0px;width: 100%; overflow: hidden; clear: both; position: relative">
...
if (!empty($_GET['Year'])) {
    $search_where['Year'] = $_GET['Year'];
}

if (!empty($_GET['Day_And_Month'])) {
    $search_where['Day_And_Month'] = $_GET['Day_And_Month'];
}

if (!empty($_GET['Month'])) {
    $search_where['Month'] = $_GET['Month'];
}

if (!empty($_GET['Occupation'])) {
    $search_where['Occupation'] = $_GET['Occupation'];
}

if (!empty($_GET['Mine'])) {
    $search_where['Mine'] = $_GET['Mine'];
}
...
if (!empty($search_where)) {
    ...
    while (list($k, $v) = each($search_where)) {
    $where[] = $k . " = '" . $v . "'";
    }
...
if (!empty($_GET['Name'])) {
    ...
    $search_name['Given_Name'] = "%" . $_GET['Name'] . "%";
    $search_name['Surname'] = "%" . $_GET['Name'] . "%";
}
...
if (!empty($search_name)) {
    ...
    while (list($k, $v) = each($search_name)) {
    $where[] = $k . " like '" . $v . "'";
    }

    $criteria[] = implode(' or ', $where);
}
...
$sql = "select *, concat_ws(' ', Given_Name, Surname) as Name from mine_accidents" . $criteria;

//echo $sql;

$count = mysqli_num_rows(mysqli_query($mysqli,$sql));

$result = MysqlFetchAssoc($mysqli, $sql);

if (empty($_GET)) {
    echo "<h3></h3>";
} else if (empty($result)) {
    echo "<h3>Sorry, no matches. Please modify your search</h3>";
} else {
    echo "<h3>We've found ".$count." matches to your search : </h1>";

    foreach ($result as $row) {
    
    
        $occupation = ucfirst($row['Occupation']);
        $description = ucfirst($row['Descript_of_accident']);
        $name = $row['Name'];
        if(strpos($name, 'Not available') === 0) { $name = substr($name,14); }
    ?>

<div class="mining-death-result">
    <strong><?php echo $name; ?></strong><br />
    <span><b>Occupation</b> : <?php echo $occupation; ?></span><br />
    <span><b>Hazard</b> : <?php echo $description . " - " . $row['Outcome']; ?></span><br />
    <span><b>Mine</b> : <?php echo $row['Mine']; ?></span><br />
    <span><b>Date</b> : <?php echo $row['Day_And_Month'].' '.$row['Year']; ?></span><br />
</div>
